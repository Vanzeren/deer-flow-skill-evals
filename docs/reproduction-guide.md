...
JUDGE_MODEL=openai-api/deepseek/deepseek-v4-flash \
...
确保 `JUDGE_MODEL` 格式正确。DeepSeek 默认用
`openai-api/deepseek/deepseek-v4-flash`；如果更看重 Judge 稳定性、可以接受更高成本，使用
`openai-api/deepseek/deepseek-v4-pro`。OpenAI 可使用 `openai/gpt-4o`。

> `deepseek-chat` 和 `deepseek-reasoner` 是旧兼容别名，将于 2026-07-24
> 15:59 UTC 停用。不要再把它们写入新配置。V4 模型支持 thinking/non-thinking，Judge
> 使用默认 thinking 模式即可。无论使用哪个模型，仍可能因未引用必需证据而产生
> `judge_failure`；证据校验不能由模型名称替代。
